...
- Pairing: room-code sync using PeerJS (WebRTC) over PeerJS's free public broker. No account, no backend of our own. Display shows a short room code (and ideally a QR code); Controller enters it. The Controller is the source of truth; Display re-syncs full state on (re)connect.
...
- **Manual draw**: a big "Draw" button picks the next random number.
...
- Sound: voice call on/off, draw sound on/off (numbers are still announced when draw sound is off), a "Clap" button for crowd engagement, and a test-voice button.
